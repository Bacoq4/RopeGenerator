...
public class HairHolder : MonoBehaviour
{
    ...
    public void drawBezierQuadraticCurve()
    {
    ...
    public float[] getPointsBtw01(int count)
    {
        ...
        float minQuant = 1 / count;
        float QuantIndexer = minQuant;

        for (int i = 0; i < count; i++)
        {
            fArr[i] = Mathf.Clamp(QuantIndexer,0,1);
            QuantIndexer += minQuant;
            print(Mathf.Clamp(QuantIndexer, 0, 1));
        }
